...
\begin{IEEEeqnarray*}{lCl}
    S_{xx}(\alpha, f) & = & \fourier \{ X_L(f)X_U^* (f) \} \\
                      & = & \fourier \{ X(f - \alpha/2)X_U^*(f + \alpha/2) \}
\end{IEEEeqnarray*}
...
multiplying the two shifted spectra together. This is significant, since a
forward FFT of the acquired data is useful for other operations, such as
channelization, as we will see in Section \ref{sec:poly_chan}.
