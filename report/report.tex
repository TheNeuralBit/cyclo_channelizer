...
The primary limitations of an Analysis Filter Banks alone are that it is limited
...
which contain that signal. This approach, described by fred harris in
...
far. Since the analysis channelizer is maximally decimated, meaning the filter
...
synthesis channelizers to reconstruct a signal of interest that aliasing is left
in the middle of the signal bandwidth.  For this reason, \cite{Harris2} utilizes
Non-Maximally Decimated Filter Banks (NMDFBs), described in the next section.

\subsection{Non-Maximally Decimated Analysis/Synthesis Filter Banks}
NMDFBs are much like the Analysis/Synthesis Filter Banks we have already
discussed, but modified to use a lower decimation factor. An excellent overview
of these strucutres can be found in \cite{Chen1}. It starts with a general
structure that can be used for arbitrary decimation rates (where $D$ is
a factor of $M$), and a realization of this general structure when $D=M/2$.
This realization, which is used in our simulation, can be seen in
Figure~\ref{fig:poly_analysis_synthesis_nmdfb}.
...
compared to the standard analysis channelizer, leading to a decimation factor
...
Using these NMDFBs in the Combined Analysis/Synthesis Filter Bank allows the
Synthesis Filter Banks to perfectly reconstruct the received signal.
